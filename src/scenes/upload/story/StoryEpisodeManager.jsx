...
const StoryEpisodeManager = () => {
  ...
  const fetchEpisodesAndParent = useCallback(async () => {
    ...
    try {
      const [episodesResponse, parentResponse] = await Promise.all([
        apiClient.get(`/stories/${storyId}/episodes`),
        apiClient.get(`/stories/${storyId}`)
      ]);
      setEpisodes(Array.isArray(episodesResponse.data.data) ? episodesResponse.data.data : []);
      setParentStory(parentResponse.data.data); 
    } catch (error) {
      showSnackbar('Failed to fetch story data. It may not exist.', 'error');
      navigate('/upload/story');
    } finally {
      setLoading(prev => ({ ...prev, page: false }));
    }
  }, [storyId, showSnackbar, navigate]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        // UPDATED: Use apiClient.put and the correct non-nested admin route
        const response = await apiClient.post(`/admin/episodes/${editingId}`, data, {
        ...
        // The create route is nested and correct according to api.php
        const response = await apiClient.post(`/admin/stories/${storyId}/episodes`, data);
  ...
  const handleDelete = async () => {
    ...
    try {
      // UPDATED: Use the correct non-nested admin route
      await apiClient.delete(`/admin/episodes/${deleteId}`);
  ...
  const toggleLock = async (episode) => {
    if (parentStory?.is_premium && !episode.is_locked) {
      showSnackbar("Unlock the parent story to manage this episode's lock status.", "info");
      return;
    }
    ...
    try {
      // UPDATED: Use the correct non-nested admin route
      const response = await apiClient.post(`/admin/episodes/${episode.id}/${endpoint}`);
      const updatedEpisode = response.data.data;
  ...
  if (loading.page || !parentStory) {
    return (
        <Box m="20px">
            <Header title="LOADING..." subtitle="Fetching story and episode data..." />
  ...
  return (
    <Box m="20px">
      <Header title={`EPISODES: ${parentStory.name}`} subtitle={`Manage episodes for this story. Parent is currently ${parentStory.is_premium ? 'PREMIUM' : 'FREE'}.`} />
      ...
      <Grid container spacing={3}>
        {episodes.map((episode) => {
            const isEffectivelyLocked = parentStory.is_premium || episode.is_locked;
            const isLockControlDisabled = parentStory.is_premium;
            return (
