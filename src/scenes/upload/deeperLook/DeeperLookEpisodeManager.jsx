...
import {
  ...
  DialogActions, Snackbar, Alert, Tooltip, Switch, FormControlLabel
} from '@mui/material';
import {
...
const DeeperLookEpisodeManager = () => {
  const { deeperLookId } = useParams();
  const navigate = useNavigate();

  const [parentDeeperLook, setParentDeeperLook] = useState(null);
  const [episodes, setEpisodes] = useState([]);
  const [formData, setFormData] = useState({ name: '', video: null, audio: null, youtube_link: '' });
  ...
  // REFINEMENT: Add this useEffect hook to clean up object URLs on unmount, preventing memory leaks.
  useEffect(() => {
    // This function will be called when the component is unmounted
    return () => {
      // Check if the preview URLs are blob URLs (from createObjectURL) and revoke them
      if (preview.video && preview.video.startsWith('blob:')) {
        URL.revokeObjectURL(preview.video);
      }
      if (preview.audio && preview.audio.startsWith('blob:')) {
        URL.revokeObjectURL(preview.audio);
      }
    };
  }, [preview.video, preview.audio]); // Dependency array ensures we have the latest preview URLs in scope
  ...
  const fetchEpisodesAndParent = useCallback(async () => {
    ...
    try {
      const [episodesResponse, parentResponse] = await Promise.all([
        apiClient.get(`/deeper-looks/${deeperLookId}/episodes`),
        apiClient.get(`/deeper-looks/${deeperLookId}`)
      ]);
      setEpisodes(Array.isArray(episodesResponse.data.data) ? episodesResponse.data.data : []);
      setParentDeeperLook(parentResponse.data.data); 
    } catch (error) {
      showSnackbar('Failed to fetch Deeper Look data. It may not exist.', 'error');
      navigate('/upload/deeper-looks');
    } finally {
      setLoading(prev => ({ ...prev, page: false }));
    }
  }, [deeperLookId, showSnackbar, navigate]);
  ...
  // REFINEMENT: Updated to revoke old object URLs before creating new ones.
  const handleFileChange = (e, fileType) => {
    const file = e.target.files[0];
    if (file) {
      if (fileType === 'video') {
        // Revoke the old URL to free up memory immediately
        if (preview.video && preview.video.startsWith('blob:')) {
            URL.revokeObjectURL(preview.video);
        }
        const newVideoUrl = URL.createObjectURL(file);
        setFormData(p => ({ ...p, video: file, youtube_link: '' }));
        setPreview(p => ({ ...p, video: newVideoUrl }));
        setIsYouTube(false);
      } else { // audio
        // Revoke the old URL to free up memory immediately
        if (preview.audio && preview.audio.startsWith('blob:')) {
        ...
        setPreview(p => ({ ...p, audio: newAudioUrl }));
      }
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    // IMPORTANT: When updating, Laravel requires a special field to recognize POST as a PUT/PATCH request for file uploads.
    // This is a common practice for forms that can't send PUT/PATCH directly.
    if (editingId) {
        data.append('_method', 'POST'); // Laravel convention for multipart/form-data updates
    }
    
    try {
      const url = editingId 
        ? `/admin/deeper-looks/${deeperLookId}/episodes/${editingId}` 
        : `/admin/deeper-looks/${deeperLookId}/episodes`;

      const response = await apiClient.post(url, data, {
          headers: { 'Content-Type': 'multipart/form-data' },
      });

      const message = editingId ? 'Episode updated!' : 'Episode created!';
      showSnackbar(message, 'success');
      
      const updatedOrNewEpisode = response.data.data;
      if (editingId) {
        setEpisodes(prev => prev.map(ep => ep.id === updatedOrNewEpisode.id ? updatedOrNewEpisode : ep));
      } else {
        setEpisodes(prev => [updatedOrNewEpisode, ...prev]);
      }
      resetForm();
    } catch (error) {
  ...
  const handleEdit = (episode) => {
    ...
    setFormData({ name: episode.name || '', video: null, audio: null, youtube_link: episode.youtube_link || '' });
  ...
  const handleDelete = async () => {
    ...
    try {
      await apiClient.delete(`/admin/deeper-looks/${deeperLookId}/episodes/${deleteId}`);
  ...
  const toggleLock = async (episode) => {
    ...
    try {
      const response = await apiClient.post(`/admin/deeper-looks/${deeperLookId}/episodes/${episode.id}/${endpoint}`);
      const updatedEpisode = response.data.data;
      setEpisodes(prevEpisodes => prevEpisodes.map(ep => ep.id === updatedEpisode.id ? updatedEpisode : ep));
  ...
  const resetForm = () => {
    // REFINEMENT: Revoke URLs on form reset as well
    if (preview.video && preview.video.startsWith('blob:')) URL.revokeObjectURL(preview.video);
    if (preview.audio && preview.audio.startsWith('blob:')) URL.revokeObjectURL(preview.audio);
    
    setFormData({ name: '', video: null, audio: null, youtube_link: '' });
    setEditingId(null);
  ...
  const isFormValid = formData.name && (isYouTube ? formData.youtube_link : (formData.video || (editingId && preview.video)));
  
  if (loading.page || !parentDeeperLook) {
    return (
        <Box m="20px">
            <Header title="LOADING..." subtitle="Fetching Deeper Look and episode data..." />
  ...
  return (
    <Box m="20px">
      <Header title={`EPISODES: ${parentDeeperLook.name}`} subtitle={`Manage episodes for this Deeper Look. Parent is currently ${parentDeeperLook.is_premium ? 'PREMIUM' : 'FREE'}.`} />
        ...
        <form onSubmit={handleSubmit}>
          <Grid container spacing={4}>
            <Grid item xs={12} md={6}>
              <TextField fullWidth label="Episode Name" name="name" value={formData.name} onChange={handleChange} required variant="filled" InputProps={{ startAdornment: (<TitleIcon color="action" sx={{ mr: 1 }} />) }} />
              <FormControlLabel control={<Switch checked={isYouTube} onChange={(e) => { setIsYouTube(e.target.checked); if (preview.video && preview.video.startsWith('blob:')) URL.revokeObjectURL(preview.video); setFormData(p=>({...p, video: null})); setPreview(p=>({...p, video: null})); }} />} label="Use YouTube Link" sx={{ mt: 2 }} />
              {isYouTube ? (
      ...
      <Grid container spacing={3}>
        {episodes.map((episode) => {
          const isEffectivelyLocked = parentDeeperLook.is_premium || episode.is_locked;
          const isLockControlDisabled = parentDeeperLook.is_premium;
          return (
          <Grid item xs={12} sm={6} md={4} key={episode.id}>
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
              <StyledCard>
                <Box sx={{ position: 'relative' }}>
                  {isEffectivelyLocked && <Chip label="PREMIUM" color="warning" size="small" sx={{ position: 'absolute', top: 10, right: 10, zIndex: 1 }} />}
                  {getYouTubeEmbedUrl(episode.youtube_link) ? <Box sx={{ position: 'relative', paddingTop: '56.25%', backgroundColor: '#000' }}><iframe title={episode.name} style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%' }} src={getYouTubeEmbedUrl(episode.youtube_link)} frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen /></Box>
                ...
                <CardContent sx={{ flexGrow: 1 }}>
                  <Tooltip title={episode.name}><Typography gutterBottom variant="h6" noWrap>{episode.name}</Typography></Tooltip>
...
export default DeeperLookEpisodeManager;