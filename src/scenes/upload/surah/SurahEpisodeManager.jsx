...
import {
  ...
  DialogActions, Snackbar, Tooltip, useTheme, Alert, Switch, FormControlLabel
} from '@mui/material';
import {
...
const SurahEpisodeManager = () => {
  ...
  const [parentSurah, setParentSurah] = useState(null);
  ...
  // No changes needed in helper functions or useEffect
  ...
  const fetchEpisodesAndParent = useCallback(async () => {
    ...
    try {
      const [episodesResponse, parentResponse] = await Promise.all([
        apiClient.get(`/surahs/${surahId}/episodes`),
        apiClient.get(`/surahs/${surahId}`)
      ]);
      setEpisodes(Array.isArray(episodesResponse.data.data) ? episodesResponse.data.data : []);
      setParentSurah(parentResponse.data.data);
    } catch (error) {
      showSnackbar('Failed to fetch surah data. It may not exist.', 'error');
      navigate('/upload/surahs');
    } finally {
      setLoading(prev => ({ ...prev, page: false }));
    }
  }, [surahId, showSnackbar, navigate]);

  useEffect(() => { fetchEpisodesAndParent() }, [fetchEpisodesAndParent]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.youtube_link) data.append('youtube_link', formData.youtube_link);
    else if (formData.video instanceof File) data.append('video', formData.video);
    if (formData.audio instanceof File) data.append('audio', formData.audio);
    
    try {
      if (editingId) {
        // --- THE FIX IS HERE ---
        // 1. Add the _method field to tell Laravel to treat this as a PUT request.
        data.append('_method', 'post');
        
        // 2. Use apiClient.post to send the request.
        const response = await apiClient.post(`/admin/surahs/${surahId}/episodes/${editingId}`, data);
        ...
        // Create logic is already correct (uses POST)
        const response = await apiClient.post(`/admin/surahs/${surahId}/episodes`, data);
  ...
  const handleDelete = async () => {
    ...
    try {
      await apiClient.delete(`/admin/surahs/${surahId}/episodes/${deleteId}`);
  ...
  const toggleLock = async (episode) => {
    if (parentSurah?.is_premium && !episode.is_locked) {
        showSnackbar("Unlock the parent Surah to manage this episode's lock status.", "info");
        return;
    }
    setLoading(prev => ({ ...prev, lock: episode.id }));
    const endpoint = episode.is_locked ? 'unlock' : 'lock';
    try {
      const response = await apiClient.post(`/admin/surahs/${surahId}/episodes/${episode.id}/${endpoint}`);
      const updatedEpisode = response.data.data;
      ...
      setLoading(prev => ({ ...prev, lock: null }));
    }
  };
  ...
  const isFormValid = formData.name && (isYouTube ? formData.youtube_link : (formData.video || preview.video));
  
  if (loading.page || !parentSurah) {
    return (
        <Box m="20px">
            <Header title="LOADING..." subtitle="Fetching surah and episode data..." />
  ...
  return (
    <Box m="20px">
      <Header title={`EPISODES: ${parentSurah.name}`} subtitle={`Manage episodes for this surah. Parent is currently ${parentSurah.is_premium ? 'PREMIUM' : 'FREE'}.`} />
      ...
      <Grid container spacing={3}>
        {episodes.map((episode) => {
          const isEffectivelyLocked = parentSurah.is_premium || episode.is_locked;
          const isLockControlDisabled = parentSurah.is_premium;
          return (
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
              <StyledCard>
                <Box sx={{ position: 'relative' }}>
                  ...
                  {getYouTubeEmbedUrl(episode.youtube_link) ? <Box sx={{ position: 'relative', paddingTop: '56.25%', backgroundColor: '#000' }}><iframe style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%' }} src={getYouTubeEmbedUrl(episode.youtube_link)} title={episode.name} frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen /></Box>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-around', p: 1, borderTop: '1px solid', borderColor: 'divider' }}>
