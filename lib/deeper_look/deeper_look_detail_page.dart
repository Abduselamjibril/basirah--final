...
class DeeperLookDetailPage extends StatefulWidget {
  ...
  const DeeperLookDetailPage({required this.deeperLook, Key? key})
      : super(key: key);
  @override
  _DeeperLookDetailPageState createState() => _DeeperLookDetailPageState();
}

class _DeeperLookDetailCacheEntry {
  final bool isUserPremium;
  final List<dynamic> episodes;
  final Map<int, bool> episodeBookmarks;
  const _DeeperLookDetailCacheEntry({
...
class _DeeperLookDetailPageState extends State<DeeperLookDetailPage> {
  ...
  ContentType get _contentType => ContentType.deeperLook;
  int get _parentId => int.parse(widget.deeperLook['id'].toString());
  Map<String, dynamic> get _content => widget.deeperLook;
  ...
  void initState() {
    super.initState();
    _logger.i(
        "DeeperLookDetailPage initialized for ID: $_parentId, Title: '${_getTitle(widget.deeperLook)}'");
  ...
  Future<void> _fetchAllInitialData({bool forceRefresh = false}) async {
    ...
    _logger.i("Fetching initial data for Deeper Look ID: $_parentId");
    setState(() {
    ...
    try {
      ...
      final bookmarkedEpisodeIds = allBookmarks
          .where((b) => b['bookmarkable_type'].endsWith('DeeperLookEpisode'))
      ...
      _logger.e("Error in _fetchAllInitialData for Deeper Look ID: $_parentId",
          e, stackTrace);
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _toggleBookmark(int episodeId) async {
    _logger.i("Toggling bookmark for deeper look episode ID: $episodeId");
    final token = _authProvider.token;
    if (token == null) {
    ...
    try {
      final message = await _bookmarkService.toggleBookmark(
          token: token,
          bookmarkableType: 'deeper_look_episode',
          bookmarkableId: episodeId);
      _logger.i(
          "Bookmark toggled successfully for deeper look episode ID: $episodeId. Message: $message");
      _uiService.showSuccessSnackbar(message);
    } catch (e, stackTrace) {
      _logger.e(
          "Error toggling bookmark for deeper look episode ID: $episodeId",
          e,
          stackTrace);
      setState(() => _episodeBookmarks[episodeId] = isCurrentlyBookmarked);
      _cache[_parentId] = _DeeperLookDetailCacheEntry(
  ...
  Future<void> _addEpisodeToPlaylist(int playlistId, int episodeId) async {
    _logger.i("Adding deeper look episode $episodeId to playlist $playlistId");
    final token = _authProvider.token;
    if (token == null) {
    ...
    try {
      await _playlistService.addEpisodeToPlaylist(
        ...
        episodeId: episodeId,
        type: _contentType, // ContentType.deeperLook
      );
      _uiService.showSuccessSnackbar('Added to playlist.');
    } catch (e) {
      _logger.e("Error adding deeper look episode to playlist", e);
  ...
  Future<void> _createNewPlaylist(String name, int episodeId) async {
    _logger.i(
        "Creating new playlist '$name' and adding deeper look episode $episodeId");
    final token = _authProvider.token;
    if (token == null) {
    ...
    try {
      ...
      _logger.e("Error creating new playlist for deeper look episode", e);
