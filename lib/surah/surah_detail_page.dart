...
class _SurahDetailPageState extends State<SurahDetailPage> {
  ...
  ContentType get _contentType => ContentType.surah;
  int get _parentId => int.parse(widget.surah['id'].toString());
  Map<String, dynamic> get _content => widget.surah;
  ...
  void initState() {
    super.initState();
    _logger.i(
        "SurahDetailPage initialized for Surah ID: $_parentId, Title: '${_getTitle(widget.surah)}'");
  ...
  Future<void> _fetchAllInitialData({bool forceRefresh = false}) async {
    ...
    _logger.i("Fetching initial data for Surah ID: $_parentId");
    setState(() {
    ...
    try {
      ...
      _logger.e("Error in _fetchAllInitialData for Surah ID: $_parentId", e,
          stackTrace);
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _addEpisodeToPlaylist(int playlistId, int episodeId) async {
    _logger.i("Adding surah episode $episodeId to playlist $playlistId");
    final token = _authProvider.token;
    if (token == null) {
    ...
    try {
      await _playlistService.addEpisodeToPlaylist(
        ...
        episodeId: episodeId,
        type: _contentType, // ContentType.surah
      );
      _uiService.showSuccessSnackbar('Added to playlist.');
    } catch (e) {
      _logger.e("Error adding surah episode to playlist", e);
  ...
  Future<void> _createNewPlaylist(String name, int episodeId) async {
    _logger
        .i("Creating new playlist '$name' and adding surah episode $episodeId");
    final token = _authProvider.token;
    if (token == null) {
    ...
    try {
      ...
      _logger.e("Error creating new playlist for surah episode", e);
          ...
          builder: (BuildContext context, StateSetter setState) {
            if (dialogPlaylists == null) {
              _playlistService
                ...
                _logger.e(
                    "Failed to load playlists for dialog", e, stackTrace);
                if (mounted) {
                ...
                child: Column(
                  ...
                  children: [
                    if (dialogIsLoading)
                      ...
                      const Padding(
                        padding: EdgeInsets.symmetric(vertical: 32.0),
                        child: Center(child: Text('You have no playlists yet.')),
                      )
                    else
                      Flexible(
