...
class _CourseDetailPageState extends State<CourseDetailPage> {
  ...
  ContentType get _contentType => ContentType.course;
  int get _parentId => int.parse(widget.course['id'].toString());
  Map<String, dynamic> get _content => widget.course;
  ...
  void initState() {
    super.initState();
    _logger.i(
        "CourseDetailPage initialized for ID: $_parentId, Title: '${_getTitle(widget.course)}'");
  ...
  // --- PLAYLIST METHODS REFACTORED FOR SAFETY ---

  // Returns true on success, false on failure.
  Future<bool> _addEpisodeToPlaylist(int playlistId, int episodeId) async {
    final token = _authProvider.token;
    if (token == null) {
      if (mounted) {
    ...
    try {
      ...
      _logger.e("Error adding course episode to playlist", e);
      return false; // Failure
    }
  }

  // Returns true on success, false on failure.
  Future<bool> _createNewPlaylist(String name, int episodeId) async {
    final token = _authProvider.token;
    if (token == null) {
      if (mounted) {
    ...
    try {
      ...
      _logger.e("Error creating new playlist for course episode", e);
      return false; // Failure
    }
  }
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return ListTile(
                              ...
                              onTap: () async {
                                final success = await _addEpisodeToPlaylist(
                                    playlist.id, episodeId);
  ...
  // --- MODIFIED: Added red color to cancel button for consistency ---
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                final success =
                    await _createNewPlaylist(controller.text.trim(), episodeId);
